...
def prepare_data(df):
    """it is used to seperate the dependent variable and independednt features
    ...
    """
    logging.info("preparing the data by segregating dependent and independent variable")
    X = df.drop("y", axis=1)
    y = df["y"]
    return X, y
...
def save_plot(df, file_name, model):
    ...
    def _create_base_plot(df):
        logging.info("creatig the base plot")
